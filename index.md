...
- **One codebase for sync and async — not two.** Route every result
  through a single branch point
  ([`then_or_now()`](https://dereckscompany.github.io/connectcore/reference/then_or_now.md)),
  write methods mode-agnostically, and choose sync or async once at
  construction. `RestClient` is mode-transparent.
...
Define a route table and wrap your code in
...
builds a response from a list (JSON-encoded), a verbatim JSON string, or
an existing response; `load_fixtures(dir)` reads a directory of real
captured `*.json` into a named route table; and
[`local_mock_api()`](https://dereckscompany.github.io/connectcore/reference/local_mock_api.md)
is the `withr::local_*` companion for use inside a `test_that()` block.
